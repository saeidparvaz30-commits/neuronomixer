...
Status: Blocked on a dead preview database breaking `npx next build` (see Blockers). The earlier npm/pnpm conflict is resolved; `npx tsc --noEmit` is green.
...
- **BLOCKING (2026-08-16, plan 01-01 Task 1): `npx next build` fails on a dead preview database.** Types and lint both pass; the build fails later, at "Collecting page data", on `/cv/[slug]`'s `generateStaticParams` calling `prisma.authorCV.findMany()`. Supabase pooler returns `(ENOTFOUND) tenant/user postgres.wbfrsrnusmoqjquvfbra not found` (code XX000 / P2039). That ref is the **preview** project from `.env.local` (eu-central-1); production in `.env` is a different ref on eu-north-1. DNS for the pooler host resolves fine, so the project itself is paused, deleted, or renamed. Pre-existing and unrelated to the route move — it was previously masked because the build aborted earlier at the type-check step, and no build has completed in this repo since Jul 22. Unlike `sitemap.ts` (whose build-time query is wrapped in try/catch), `/cv/[slug]`'s `generateStaticParams` has no fallback, so a dead DB is fatal to the build. **Phase 01 cannot proceed:** ROUTE-02 is proven by diffing `.next/app-path-routes-manifest.json`, which is never emitted without a successful build. Needs Saeid to resume/restore the preview Supabase project or supply a working preview `DATABASE_URL`. Do NOT point the build at production to work around this.
