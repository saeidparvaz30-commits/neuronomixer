    ...
    <div className="min-h-screen pb-20">
      <div className="max-w-[900px] mx-auto px-5 sm:px-8 py-8">
        ...
        <nav className="flex items-center gap-1.5 text-[12px] text-[#94a3b8] mb-6 flex-wrap">
          ...
          <span className="text-white/20">/</span>
          <span className="text-white">Temperature &amp; Top-K: Controlling Creativity</span>
        </nav>
        ...
        <section className="mb-8">
          ...
          <h1 className="text-4xl sm:text-5xl font-black tracking-tight text-white mb-3">
            Temperature &amp; <span className="text-[#3bb4a4]">Top-K</span>
          </h1>
        ...
        <div className="flex items-center gap-3 mb-8 flex-wrap">
          {progress.map(({ label, done }) => (
            <div key={label} className="flex items-center gap-1.5">
              <motion.div animate={{ backgroundColor: done ? "#3bb4a4" : "#1e293b" }} transition={{ duration: 0.4 }}
        ...
        <section className="mb-10">
          ...
          <p className="text-[12px] text-[#94a3b8] mb-4">
            <span className="font-mono text-white">P(i) = exp(logit_i / T) / Σ exp(logit_j / T)</span> — higher T flattens distribution.
          </p>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border ${
                  ...
                  : temp <= 1.2 ? "border-[#d4af37]/40 text-[#d4af37] bg-[#d4af37]/10"
        ...
        <section className="mb-10">
          ...
          <div className="rounded-2xl border border-[#1e293b] bg-[#0f172a] p-5 space-y-2.5">
            {BASE_LOGITS.map(({ token }, i) => (
              <Bar key={token} token={token} pct={topKProbs[i] * 100} color="#d4af37" dim={topKProbs[i] === 0} />
            ))}
        ...
        <div className="rounded-2xl border border-[#1e293b] bg-[#0f172a] p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          ...
          <Link href="/visual-guides/context-windows"
            className="shrink-0 px-5 py-2 rounded-xl text-[13px] font-semibold bg-[#ef4444] text-white hover:opacity-90 transition-opacity">
            Next Guide →
          </Link>
        </div>
        ...
        <div className="flex items-center justify-between pt-6 border-t border-white/[0.06]">
          ...
          <Link href="/visual-guides/context-windows"
            className="px-5 py-2 rounded-xl text-sm font-semibold bg-[#ef4444] text-white hover:opacity-90 transition-opacity">
            Next Guide →
          </Link>
        </div>
