    ...
    <div className="rounded-2xl border border-[#1e293b] bg-[#0f172a] p-5">
      ...
      <div className="mb-4">
        <div className="flex justify-between mb-1">
          ...
          <span className="text-[11px] font-mono text-[var(--color-accent)]">{effectSize.toFixed(1)}</span>
        </div>
        <input
          aria-label="Effect size (Cohen's d)" type="range" min={0} max={2.0} step={0.1} value={effectSize}
          onChange={e => onEffectSizeChange(Number(e.target.value))}
          className="w-full appearance-none h-1.5 rounded-full cursor-pointer"
          style={{
            background: `linear-gradient(to right, var(--color-accent) ${pct(effectSize, 0, 2)}, #1e293b ${pct(effectSize, 0, 2)})`,
          }}
        />
      ...
      <div className="mb-4">
        <div className="flex justify-between mb-1">
          ...
          <span className="text-[11px] font-mono text-[var(--color-accent)]">{sampleSize}</span>
        </div>
        <input
          aria-label="Sample size per group" type="range" min={10} max={200} step={10} value={sampleSize}
          onChange={e => onSampleSizeChange(Number(e.target.value))}
          className="w-full appearance-none h-1.5 rounded-full cursor-pointer"
          style={{
            background: `linear-gradient(to right, var(--color-accent) ${pct(sampleSize, 10, 200)}, #1e293b ${pct(sampleSize, 10, 200)})`,
          }}
        />
      ...
      <div className="mb-5">
        <div className="flex justify-between mb-1">
          ...
          <span className="text-[11px] font-mono text-[var(--color-accent)]">{alpha.toFixed(2)}</span>
        </div>
        <input
          aria-label="Significance level (alpha)" type="range" min={0.01} max={0.20} step={0.01} value={alpha}
          onChange={e => onAlphaChange(Number(e.target.value))}
          className="w-full appearance-none h-1.5 rounded-full cursor-pointer"
          style={{
            background: `linear-gradient(to right, var(--color-accent) ${pct(alpha, 0.01, 0.20)}, #1e293b ${pct(alpha, 0.01, 0.20)})`,
          }}
        />
      ...
      <div className="mb-5">
        ...
        <div className="flex gap-1.5" role="radiogroup" aria-label="Test type">
