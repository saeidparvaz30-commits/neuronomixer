    ...
    if (isComplete && !completionFired.current) {
      ...
      if (session?.user) {
        fetch("/api/visual-guides/complete", {
          ...
          body: JSON.stringify({ guideSlug: "confusion-matrix", score: 8 }),
        }).catch(() => {});
      }
    }
    ...
    <div className="min-h-screen bg-[#0f172a] text-white">
      ...
      <div className="max-w-[1300px] mx-auto px-5 sm:px-8 lg:px-10 py-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-[#94a3b8] mb-6">
          ...
          <span>/</span>
          <span className="text-white">The Confusion Matrix Decoded</span>
        </nav>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-6">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="bg-[#1e293b]/60 border border-[#1e293b] rounded-2xl p-5">
              <h3 className="text-sm font-semibold text-white mb-4">Confusion Matrix — threshold = {threshold.toFixed(2)}</h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                ...
                { label: "Precision", value: precision, desc: "TP/(TP+FP)", color: "#d4af37" },
              ...
              <div className="flex gap-2 mt-3">
                {[0.3, 0.5, 0.7].map(t => (
                  <button key={t} onClick={() => { setThreshold(t); setThresholdMoves(p => p + 1); }}
                    className={`flex-1 px-2 py-1 rounded text-xs border transition-all ${Math.abs(threshold - t) < 0.02 ? "bg-[#d4af37]/20 border-[#d4af37]/60 text-[#d4af37]" : "border-[#334155] text-[#94a3b8]"}`}
                  >
                    {t}
                  </button>
                ))}
            ...
            <div className="bg-[#1e293b]/60 border border-[#1e293b] rounded-xl p-4">
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="flex items-center gap-2">
                  <div className="w-16 text-[#94a3b8] flex-shrink-0">↑ threshold</div>
                  <div className="flex-1">
                    <div className="text-[#d4af37]">↑ Precision (fewer false alarms)</div>
        ...
        <div className="flex items-center justify-between mt-10 pt-6 border-t border-[#1e293b]">
          <Link href="/visual-guides/cross-validation" className="flex items-center gap-2 text-sm text-[#94a3b8] hover:text-white transition-colors">
            <span>←</span><span>Cross-Validation</span>
          </Link>
          ...
          <Link href="/visual-guides/roc-curves" className="flex items-center gap-2 text-sm text-[#94a3b8] hover:text-white transition-colors">
            <span>ROC Curves</span><span>→</span>
          </Link>
        </div>
      </div>
    </div>
