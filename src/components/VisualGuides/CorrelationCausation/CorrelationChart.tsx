...
function CorrelationChartInner({ points, xLabel, yLabel, xRange, yRange, showRegression = true, rValue, compact }: Props) {
  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${compact ? 240 : H}`} className="block"
      ...
      {/* Ticks */}
      {xTicks.map((v, i) => <text key={`xl-${i}`} x={toSvgX(v, ...xRange)} y={PAD.t + IH + 14} textAnchor="middle" fontSize="14" fill="#475569" fontFamily="Inter,sans-serif">{fmt(v)}</text>)}
      {yTicks.map((v, i) => <text key={`yl-${i}`} x={PAD.l - 5} y={toSvgY(v, ...yRange) + 5} textAnchor="end" fontSize="14" fill="#475569" fontFamily="Inter,sans-serif">{fmt(v)}</text>)}

      {/* Axis labels */}
      <text x={PAD.l + IW / 2} y={W > 300 ? H - 2 : 235} textAnchor="middle" fontSize="15" fill="#94a3b8" fontFamily="Inter,sans-serif">{xLabel}</text>
      <text x={12} y={PAD.t + IH / 2} textAnchor="middle" fontSize="15" fill="#94a3b8" fontFamily="Inter,sans-serif" transform={`rotate(-90,12,${PAD.t + IH / 2})`}>{yLabel}</text>
          ...
          <g key={i}>
            ...
            {isHov && (
              <g style={{ pointerEvents: "none" }}>
                <rect x={cx - 55} y={cy - 40} width="110" height="26" rx="4" fill="#0f172a" stroke="#334155" strokeWidth="1" />
                <text x={cx} y={cy - 22} textAnchor="middle" fontSize="14" fill="#f1f5f9" fontFamily="Inter,sans-serif">
                  ({fmt(pt.x)}, {fmt(pt.y)})
                </text>
              </g>
            )}
      ...
      {/* R value */}
      {rValue !== undefined && (
        <text x={PAD.l + IW - 4} y={PAD.t + 14} textAnchor="end" fontSize="15" fontWeight="600" fill="var(--color-accent)" fontFamily="Inter,sans-serif">
          r = {rValue.toFixed(2)}
        </text>
      )}
    </svg>
