...
export default function CausalExplainer() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      {/* Causation */}
      <div className="rounded-2xl border border-[var(--color-accent)]/30 bg-[var(--color-accent)]/5 p-5">
        <div className="flex items-center gap-2 mb-3">
          <svg width="34" height="22" viewBox="0 0 34 22">
            <line x1="4" y1="11" x2="26" y2="11" stroke="var(--color-accent)" strokeWidth="2" />
            <polygon points="26,6 33,11 26,16" fill="var(--color-accent)" />
          </svg>
          <p className="text-[14px] font-bold text-[var(--color-accent)]">Causation</p>
        </div>
        <p className="text-[12px] text-[#94a3b8] mb-3">One variable directly causes changes in another via a mechanism.</p>
        <div className="rounded-lg bg-[#1e293b]/60 p-2.5 mb-3">
          <p className="text-[11px] font-mono text-[var(--color-accent)]">Temperature → Ice Cream Sales</p>
          <p className="text-[10px] text-[#475569] mt-0.5">Directional: mechanism exists</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[22px] font-black text-[var(--color-accent)]">→</span>
      ...
      <div className="sm:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          ...
          <div key={title} className="rounded-xl bg-[#1e293b]/40 border border-[#1e293b] p-3">
            ...
            <p className="text-[9px] font-mono text-[var(--color-accent)] mb-1.5">{symbol}</p>
