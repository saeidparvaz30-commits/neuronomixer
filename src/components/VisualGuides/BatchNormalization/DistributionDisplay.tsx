      ...
      <div className="flex items-end gap-[2px] h-20 w-full">
        <AnimatePresence mode="wait">
          {values.map((v, i) => {
            ...
            const normalizedPos = Math.abs(v - dist.mean) / (dist.std || 1);
            // Color: turquoise near mean, fades toward extremes
    ...
    >
      ...
      <p className="mt-4 text-xs text-[#94a3b8] leading-relaxed">
        {withBN
          ...
          : "Each layer develops a different distribution. Layer 4 has mean=2.1 and std=2.0 — forcing subsequent layers to constantly re-adapt."}
      </p>
    </div>
  );
}
